...
bool Win32Window::Initialize(const WindowConfig &config) {
  ...
  // This is the key part - window initializes input after it has a valid handle
...
bool Win32Window::RegisterWindowClass() {
  ...
  if (result == 0) {
    ...
    case ERROR_INVALID_PARAMETER:
      std::cerr
          << "ERROR_INVALID_PARAMETER (1400) - One of the parameters is invalid"
          << std::endl;
      std::cerr
          << "Check: window class, style flags, size values, instance handle"
          << std::endl;
      break;
    case ERROR_CLASS_DOES_NOT_EXIST:
      std::cerr << "ERROR_CLASS_DOES_NOT_EXIST - Window class not found"
                << std::endl;
      break;
    // case ERROR_INVALID_WINDOW_HANDLE: // This is 1400 too, but different
    // context
  ...
  if (!hwnd) {
    ...
    case ERROR_INVALID_PARAMETER:
      std::cerr
          << "ERROR_INVALID_PARAMETER (1400) - One of the parameters is invalid"
          << std::endl;
      std::cerr << "Check: window class registration, style flags, size values"
                << std::endl;
      break;
    case ERROR_CLASS_DOES_NOT_EXIST:
      std::cerr << "ERROR_CLASS_DOES_NOT_EXIST - Window class not found"
    ...
    case ERROR_INVALID_WINDOW_HANDLE:
      std::cerr << "ERROR_INVALID_WINDOW_HANDLE - Invalid parent window handle"
                << std::endl;
      break;
    case ERROR_NOT_ENOUGH_MEMORY:
      std::cerr << "ERROR_NOT_ENOUGH_MEMORY - Insufficient memory" << std::endl;
      break;
    default:
...
std::string Win32Window::GetTitle() const { return m_config.title; }
...
WindowHandle Win32Window::GetNativeHandle() const { return m_hwnd; }

std::shared_ptr<IInput> Win32Window::GetInput() const { return m_input; }
...
void Win32Window::RequestClose() { m_shouldClose = true; }
...
void Win32Window::SetPosition(int x, int y) { /* Implementation needed */ }
void Win32Window::GetPosition(int &x,
                              int &y) const { /* Implementation needed */ }
...
bool Win32Window::IsMinimized() const { return false; }
bool Win32Window::IsMaximized() const { return false; }
bool Win32Window::HasFocus() const { return m_hwnd == GetForegroundWindow(); }
void Win32Window::SetFullscreen(bool fullscreen) {
  m_isFullscreen = fullscreen;
}
bool Win32Window::IsFullscreen() const { return m_isFullscreen; }
void Win32Window::SetVSync(bool enabled) { m_vsyncEnabled = enabled; }
bool Win32Window::IsVSyncEnabled() const { return m_vsyncEnabled; }
...
void Win32Window::SetIcon(
    const std::string &iconPath) { /* Implementation needed */ }
void Win32Window::ClientToScreen(int &x,
                                 int &y) const { /* Implementation needed */ }
void Win32Window::ScreenToClient(int &x,
                                 int &y) const { /* Implementation needed */ }