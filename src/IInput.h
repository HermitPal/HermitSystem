...
using MouseButtonCallback = std::function<void(MouseButton button, bool pressed, int x, int y)>;
...
class IInput
{
  public:
    ...
    virtual bool WasKeyPressed(Key key) const = 0;  // True for one frame when key is first pressed
    virtual bool WasKeyReleased(Key key) const = 0; // True for one frame when key is released
