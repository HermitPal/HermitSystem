...
Win32Input::~Win32Input() { Shutdown(); }
...
bool Win32Input::IsKeyUp(Key key) const { return !IsKeyDown(key); }
...
bool Win32Input::IsMouseButtonDown(MouseButton button) const {
  size_t buttonIndex = static_cast<size_t>(button);
  return (buttonIndex < MAX_MOUSE_BUTTONS) ? m_currentMouseState[buttonIndex]
                                           : false;
}
...
bool Win32Input::WasMouseButtonPressed(MouseButton button) const {
  ...
  return m_currentMouseState[buttonIndex] && !m_previousMouseState[buttonIndex];
}
...
int Win32Input::GetMouseWheelDelta() const { return m_wheelDelta; }
...
void Win32Input::ProcessMessage(unsigned int message, uintptr_t wParam,
  ...
  case WM_KEYDOWN:
  case WM_SYSKEYDOWN:
    HandleKeyDown(static_cast<WPARAM>(wParam), static_cast<LPARAM>(lParam));
    break;
  ...
  case WM_MOUSEMOVE:
    HandleMouseMove(static_cast<WPARAM>(wParam), static_cast<LPARAM>(lParam));
    break;

  case WM_MOUSEWHEEL:
    HandleMouseWheel(static_cast<WPARAM>(wParam), static_cast<LPARAM>(lParam));
    break;
  }
}
...
void Win32Input::HandleMouseButtonDown(UINT message, WPARAM wParam,
                                       LPARAM lParam) {
...
void Win32Input::HandleMouseButtonUp(UINT message, WPARAM wParam,
                                     LPARAM lParam) {
  MouseButton button = MouseButton::Left;
...
bool Win32Input::IsKeyPressed(Key key) const { return WasKeyPressed(key); }
